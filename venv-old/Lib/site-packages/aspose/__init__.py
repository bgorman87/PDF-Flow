...
class AsposeModuleLoader:
    ...
    def _load_native_module(self, native_name):
        for ext in _platform_suffixes:
            lib_file = os.path.join(self.aspose_native_lib_dir, native_name + ext)
    ...
    def _import_file(module, name, path):
        try:
            ...
            src_module = imp.load_source(private_name, path)
    ...
    def _register_additional_classes(self, module, sub_lib):
        ...
        if os.path.isdir(folder):
            foreach_file( folder, im.SOURCE_SUFFIXES, lambda name, path, module=module: AsposeModuleLoader._import_file(module, name, path) )
...
os.environ["CORE_ROOT"] = os.path.join(AsposeModuleLoader.aspose_native_lib_dir, "netcore")
os.environ["CORE_LIBRARIES"] = os.path.join(AsposeModuleLoader.aspose_native_lib_dir, "netcore")
...
__all__ = []
foreach_file(AsposeModuleLoader.aspose_native_lib_dir, _platform_suffixes, lambda name, path: __all__.append(name))
